...
class maldocAnalyzerPdf:
    def __init__(self):
        self.readCVS()
        self.inserFileSearch()
        
        
        self.cleanAuxFile()
    def inserFileSearch(self):
        ...
        vURI=listFile[47]
        vColor=listFile[51]
        self.searchMalware(vHeader,Vobj,Vendobj,float(Vstream),Vendstream,Vxref,float(Vtrailer),Vstartxref,Vpage,float(Vencrypt),float(VobjStm),vJS,VJavaScript,vAA,VopenAction,VAcroForm,VJBIG2Decode,VRichMedia,Vlaunch,VEmbeddedFile,vXFA,float(vColor))
    ...
    def searchMalware(self,header,obj,endobj,stream,endstream,xref,trailer,startxref,pageno,encrypt,ObjStm,JS,Javascript,
        ...
        if(value['Class'].empty):
            print("there are no matches the pdf file is in an insecure state")
            print("reviewing structure to determine risk")
            self.analizerPdf(OpenAction,AA,Javascript,JS,launch,RichMedia,ObjStm,JBIG2Decode,)
        else:
            print("the pdf documents is:  ",value["Class"].values[0])
    def analizerPdf(self,OpenAction,AA,Javascript,JS,launch,RichMedia,ObjStm,JBIG2Decode):
        ...
        if(vl):
            print(  "the document is very suspicious, possibly it is maldoc")
        elif(RichMedia>0 or ObjStm>0):
            print(" nothing directly malicious was found, but the document could be hiding its intentions in:",end="")
            print("RichMedia=  ",RichMedia,"objStm=   ",ObjStm)
        elif((Javascript>0 or JS>0 or launch>0 )):
            print(" the document is running some kind of javascript or JS, so we advise you to proceed carefully if it asks you to activate or open something")
        else:
            print(" nothing suspicious in this document")
